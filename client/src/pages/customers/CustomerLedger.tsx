...
interface Props {
  customer: { id: number; name: string; outstandingBalance?: string }
  onClose: () => void
}
...
export default function CustomerLedger({ customer, onClose }: Props) {
  ...
  const payMutation = useMutation({
    mutationFn: (payload: { amount: number; method: string; reference?: string }) =>
      api.post(`/customers/${customer.id}/payments`, payload).then(r => r.data),
    onSuccess: () => {
  ...
  // Backend returns { customer, ledger, closingBalance }
  ...
  // Closing balance: last entry's balance in filtered range, else full
  const closingBalance = entries.length > 0
    ? Number(entries[entries.length - 1]?.balance ?? 0)
    : (dateFrom || dateTo ? 0 : data?.closingBalance ?? 0)

  const totalSales = entries.filter(e => e.type === 'SALE').reduce((s: number, e: any) => s + Number(e.debit), 0)
  const totalPaid  = entries.filter(e => e.type === 'PAYMENT' || e.type === 'SALE_RETURN').reduce((s: number, e: any) => s + Number(e.credit), 0)

  const typeStyle = (type: string) => {
    if (type === 'SALE') return { label: 'Sale', color: 'var(--red-risk)' }
    if (type === 'PAYMENT') return { label: 'Payment', color: 'var(--green-ok)' }
    if (type === 'SALE_RETURN') return { label: 'Return', color: '#7C3AED' }
        ...
        <div className="space-y-4">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            <div style={{ borderRadius: 'var(--radius)', padding: '10px 14px', textAlign: 'center', background: 'var(--paper)', border: '1px solid var(--rule)' }}>
              <div style={{ fontSize: 11, color: 'var(--steel)', marginBottom: 4 }}>Total Billed</div>
              <div style={{ fontWeight: 700, fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--ink)' }}>Rs. {totalSales.toLocaleString()}</div>
            </div>
            ...
            <div style={{ borderRadius: 'var(--radius)', padding: '10px 14px', textAlign: 'center', background: closingBalance > 0 ? '#FBE7E2' : '#E4F5EC', border: closingBalance > 0 ? '1px solid #F4C2B8' : '1px solid #DCEFE6' }}>
              <div style={{ fontSize: 11, color: closingBalance > 0 ? '#C1462F' : '#2F8F5F', marginBottom: 4 }}>Outstanding</div>
          ...
          {payOpen && (
            <div style={{ background: 'var(--paper-light)', border: '1px solid var(--rule)', borderRadius: 'var(--radius)', padding: '12px 14px' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', marginBottom: 10 }}>Receive Payment</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            {closingBalance > 0 && !payOpen && (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 'var(--radius)', border: '1px solid var(--green-ok)', background: 'rgba(62,142,90,0.08)', color: 'var(--green-ok)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}
              >
                + Receive Payment
              </button>
            )}
